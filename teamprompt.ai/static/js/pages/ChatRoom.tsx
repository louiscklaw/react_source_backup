...
const ChatRoom: React.FC = () => {
  ...
  useEffect(() => {
    if (conversationId && conversation?.conversationId !== conversationId) {
      apiService
        .get<{ conversation: Conversation; messages: Message[] }>(`/v1/conversation/conversations/${conversationId}`)
  ...
  const sendMessageStream = async (message: string, onComplete?: () => void) => {
    ...
    await apiService.fetchSSE(
      isCreateChat ? `/v1/conversation/conversations` : `/v1/conversation/conversations/${conversationId}/messages`,
